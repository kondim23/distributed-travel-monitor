...
char tempString[20], fifoName[2][300], *subdirectory, *inputDir, *lineInput=NULL, bufferLine[200], *token, date[11], running=1;
...
void terminateProgram() {
    ...
    int stat_val, fd;
    pid_t pid;
    char *tempstring = (char*) malloc(sizeof(LOGS_DIR)+16);
    // char tempstring[16];
    ...
    strcpy(tempstring,LOGS_DIR);
    strcat(tempstring,"log_file.");
...
void create_pipes_and_monitors(int mon){
    ...
    char binpath[100];
    strcpy(binpath,BIN_DIR);

    /*Create 2 pipes for read - wrirte for given monitor*/
    strcpy(fifoName[READ],PIPES_DIR);
    sprintf(fifoName[READ]+strlen(fifoName[READ]), "mon%d_to_tm",mon);
    ...
    strcpy(fifoName[WRITE],PIPES_DIR);
    sprintf(fifoName[WRITE]+strlen(fifoName[WRITE]), "tm_to_mon%d",mon);
    ...
    switch(pid) {
        ...
        case 0:
            ...
            free(bloomHashes);
            hash_destroy(requestsHash);
            execlp(strcat(binpath,"Monitor"),fifoName[READ],fifoName[WRITE],NULL);
    }
